...
<body>
        ...
        <tbody>
            @foreach($transkrip as $index => $item)
                <tr>
                    ...
                    <td style="text-align: center;">{{ $item->periodeAkademik->semester }}</td>
    ...
    <div class="summary">
        <table>
            <tr>
                <td width="70%">
                    <strong>Jumlah Mata Kuliah: {{ $transkrip->count() }}</strong>
                </td>
                <td width="30%">
                    <strong>Total SKS: {{ $transkrip->sum('sks') }}</strong>
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    <strong>Indeks Prestasi Kumulatif (IPK): {{ number_format($ipk, 2) }}</strong>
                </td>
            </tr>
        </table>
    </div>
